...
|   d    |                                                             无占位0的日期 (1 to 31)                                                              |
|   dd   |                                                             有占位0的日期 (01 to 31)                                                             |
|  ddd   |    简写的一周七天名称(e.g. 'Mon' to 'Sun'). 使用系统地域设置来格式化, 也就是 [QLocale::system](https://doc.qt.io/qt-5/qlocale.html#system)()     |
|  dddd  | 长版的一周七天名称 (e.g. 'Monday' to 'Sunday'). 使用系统地域设置来格式化, 也就是 [QLocale::system](https://doc.qt.io/qt-5/qlocale.html#system)() |
|   M    |                                                             无占位0的月份 (1 to 12)                                                              |
|   MM   |                                                             有占位0的月份 (01 to 12)                                                             |
|  MMM   |      缩写的月份名称 (e.g. 'Jan' to 'Dec'). 使用系统地域设置来格式化, 也就是 [QLocale::system](https://doc.qt.io/qt-5/qlocale.html#system)()      |
|  MMMM  | 长版的月份名称 (e.g. 'January' to 'December'). 使用系统地域设置来格式化, 也就是 [QLocale::system](https://doc.qt.io/qt-5/qlocale.html#system)()  |
|   yy   |                                                              两位数年份 (00 to 99)                                                               |
|  yyyy  |                                                   四位数年份。 如果是负数，加上符号是五个字符                                                    |

单引号包裹的内容将会一字不差地放进输出到字符串中 (不带外面的单引号), 尽管包含上述格式占位符. 连续两个单引号('')会被转义成一个单引号输出. 所有其他字符不会被转义，将原封不动输出。

没有分隔符的多个占位符(e.g. "ddMM")受支持，但要慎用。因为结果的可读性不好，易引起多义(例如“dM”的输出"212"就分不清2.12还是21,2)
...
**看看别的：** [day](https://doc.qt.io/qt-5/qdate.html#day-1)() and [daysInYear](https://doc.qt.io/qt-5/qdate.html#daysInYear-1)().
...
注意 [Qt::TextDate](https://doc.qt.io/qt-5/qt.html#DateFormat-enum): 建议使用英文的简写月份名称(e.g. "Jan"). Although localized month names can also be used in Qt 5, they depend on the user's locale settings.
