...
QMap 类是一种基于红黑树结构的模板类。[更多...](QMap.md#详细描述)
...
| typedef | **[const_key_value_iterator](QMap.md#typedef-qmapconstkeyvalueiterator)** |
| typedef | **[difference_type](QMap.md#typedef-qmapdifferencetype)**    |
| typedef | **[key_type](QMap.md#typedef-qmapkeytype)**                  |
| typedef | **[key_value_iterator](QMap.md#typedef-qmapkeyvalueiterator)** |
| typedef | **[mapped_type](QMap.md#typedef-qmapmappedtype)**            |
| typedef | **[size_type](QMap.md#typedef-qmapsizetype)**                |
...
|                              QMap::const_iterator | **[begin](QMap.md#qmapconstiterator-qmapbegin-const)**() const |
|                              QMap::const_iterator | **[cbegin](QMap.md#qmapconstiterator-qmapcbegin-const)**() const |
|                              QMap::const_iterator | **[cend](QMap.md#qmapconstiterator-qmapcend-const)**() const |
|                                              void | **[clear](QMap.md#void-qmapclear)**()                        |
|                              QMap::const_iterator | **[constBegin](QMap.md#qmapconstiterator-qmapconstbegin-const)**() const |
|                              QMap::const_iterator | **[constEnd](QMap.md#qmapconstiterator-qmapconstend-const)**() const |
|                              QMap::const_iterator | **[constFind](QMap.md#qmapconstiterator-qmapconstfindconst-key-key-const)**(const Key &*key*) const |
|                    QMap::const_key_value_iterator | **[constKeyValueBegin](QMap.md#qmapconstkeyvalueiterator-qmapconstkeyvaluebegin-const)**() const |
|                    QMap::const_key_value_iterator | **[constKeyValueEnd](QMap.md#qmapconstkeyvalueiterator-qmapconstkeyvalueend-const)**() const |
...
|                              QMap::const_iterator | **[end](QMap.md#qmapconstiterator-qmapend-const)**() const   |
...
| QPair<QMap::const_iterator, QMap::const_iterator> | **[equal_range](QMap.md#qpairqmapconstiterator-qmapconstiterator-qmapequalrangeconst-key-key-const)**(const Key &*key*) const |
...
|                              QMap::const_iterator | **[find](QMap.md#qmapconstiterator-qmapfindconst-key-key-const)**(const Key &*key*) const |
...
|                                    QMap::iterator | **[insert](QMap.md#qmapiterator-qmapinsertqmapconstiterator-pos-const-key-key-const-t-value)**(QMap::const_iterator *pos*, const Key &*key*, const T &*value*) |
...
|                                QMap::key_iterator | **[keyBegin](QMap.md#qmapkeyiterator-qmapkeybegin-const)**() const |
|                                QMap::key_iterator | **[keyEnd](QMap.md#qmapkeyiterator-qmapkeyend-const)**() const |
|                          QMap::key_value_iterator | **[keyValueBegin](QMap.md#qmapkeyvalueiterator-qmapkeyvaluebegin)**() |
|                    QMap::const_key_value_iterator | **[keyValueBegin](QMap.md#qmapconstkeyvalueiterator-qmapkeyvaluebegin-const)**() const |
|                          QMap::key_value_iterator | **[keyValueEnd](QMap.md#qmapkeyvalueiterator-qmapkeyvalueend)**() |
|                    QMap::const_key_value_iterator | **[keyValueEnd](QMap.md#qmapconstkeyvalueiterator-qmapkeyvalueend-const)**() const |
...
|                              QMap::const_iterator | **[lowerBound](QMap.md#qmapconstiterator-qmaplowerboundconst-key-key-const)**(const Key &*key*) const |
...
|                              QMap::const_iterator | **[upperBound](QMap.md#qmapconstiterator-qmapupperboundconst-key-key-const)**(const Key &*key*) const |
...
QMap<Key, T> 是一种 Qt 泛型[容器类](../../C/Container_Classes/Container_Classes.md)。该类内部存储键值对，可以用相关联的键快速查找值。
...
如果 map 中不存在指定的键，这些函数返回[默认构造出来的值](../../C/Container_Classes/Container_Classes.md#容器类)。

如果想查看 map 中是否含有某个键，使用[contains](QMap.md#bool-qmapcontainsconst-key-key-const)()：
...
还有一个 [value](QMap.md#const-t-qmapvalueconst-key-key-const-t-defaultvalue--t-const)() 的重载函数，如果没有指定键的元素，该函数使用其第2个参数作为默认值：

```c++
int timeout = map.value("TIMEOUT", 30);
```

通常情况下，推荐使用 [contains](QMap.md#bool-qmapcontainsconst-key-key-const)() 和 [value](QMap.md#const-t-qmapvalueconst-key-key-const-t-defaultvalue--t-const)() 而不是 operator\[\]\(\) 运算符来查找 map 中的键。原因是如果 map 中不存在该键的元素，operator\[\]\(\) 运算符会默默地将一个元素插入到 map 中（除非 map 是 const 的）。例如，下面的代码片段将在内存创建1000个元素：
...
如果想遍历 QMap 中存储的所有键值对，可以使用迭代器。QMap 同时提供 [Java 风格迭代器](../../C/Container_Classes/Container_Classes.md#Java-风格迭代器)（[QMapIterator](../../M/QMapIterator/QMapIterator.md) 和 [QMutableMapIterator](../../M/QMutableMapIterator/QMutableMapIterator.md)）和[STL 风格迭代器](../../C/Container_Classes/Container_Classes.md#STL-风格迭代器)（[QMap::const_iterator](../../M/QMap/QMap-const-iterator.md) 和 [QMap::iterator](../../M/QMap/QMap-iterator.md)）。下面是使用Java风格的迭代器遍历 QMap<[QString](../../S/QString/QString.md), int> 的方法：
...
然而，可以使用派生类 [QMultiMap](../../M/QMultiMap/QMultiMap.md) 在一个键中存储多个值。使用 values(const Key &key) 取得单个键的所有值，该函数返回一个 [QList](../../L/QList/QList.md)\<T\>：
...
同一个键的多个值按照从最新到最早插入的顺序返回。另外一种方法是调用 [find](QMap.md#qmapiterator-qmapfindconst-key-key)() 取得STL风格的迭代器，该迭代器指向键相同的多个元素的首元素，然后从该元素开始遍历：
...
如果只想从 map 中取值，也可以使用 [foreach](../../C/Container_Classes/Container_Classes.md#foreach-关键字)：
...
QMap 键和值的数据类型必须是[可赋值数据类型](../../C/Container_Classes/Container_Classes.md#容器类)。这涵盖了大多数您可能会遇到的数据类型，但是编译器不会存储像 [QWidget](../../W/QWidget/QWidget.md) 这样的对象作为值，应该存储 [QWidget](../../W/QWidget/QWidget.md) *。另外，QMap 的键类型必须提供 operator<() 运算符。QMap 用它来保持元素有序，并假定两个键 `x` 和 `y` 在 `x < y` 和 `y < x` 都不为 true 的情况下相等。
...
在该例中，先比较雇员名。如果雇员名相等，再比较二者的生日来分出大小。
...
QMap::const_key_value_iterator 为 [QMap](../../M/QMap/QMap.md) 和 [QMultiMap](../../M/QMultiMap/QMultiMap.md) 提供 STL 风格的迭代器。

QMap::const_key_value_iterator 类型定义除了 operator *() 运算符返回的是键值对而不是值以外，其它基本和 [QMap::const_iterator](../../M/QMap/qmap-const-iterator.md) 一样。
...
为兼容 STL 提供的 ptrdiff_t 的类型别名。

### typedef QMap::key_type

为兼容 STL 提供的 Key 的类型别名。

### typedef QMap::key_value_iterator

QMap::key_value_iterator 为 [QMap](../../M/QMap/QMap.md) 和 [QMultiMap](../../M/QMultiMap/QMultiMap.md) 提供 STL 风格的迭代器。

QMap::key_value_iterator 类型定义除了 operator *() 运算符返回的是键值对而不是值以外，其他基本和 [QMap::iterator](../../M/QMap/qmap-iterator.md) 一样。
...
该操作为[常数时间复杂度](../../C/Container_Classes/Container_Classes.md#算法复杂度)，因为 QMap 是[隐式共享](../../I/Implicit-sharing/Implicit-sharing.md)的。这使得从一个函数返回 QMap 非常快。如果共享实例被修改了，它将以[线性时间](../../C/Container_Classes/Container_Classes.md#算法复杂度)被复制一份（写时拷贝）。
...
**另请参阅** [constBegin](QMap.md#qmapconstiterator-qmapconstbegin-const)() 和 [end](QMap.md#qmapiterator-qmapend)()。
...
**另请参阅** [begin](QMap.md#qmapiterator-qmapbegin)() and [cend](QMap.md#qmapconstiterator-qmapcend-const)()。
...
**另请参阅** [cbegin](QMap.md#qmapconstiterator-qmapcbegin-const)() 和 [end](QMap.md#qmapiterator-qmapend)()。
...
**另请参阅** [begin](QMap.md#qmapiterator-qmapbegin)() 和 [constEnd](QMap.md#qmapconstiterator-qmapconstend-const)()。
...
返回常量类型的迭代器指向 map 中键为 *key* 的元素。

如果 map 中没有键为 *key* 的元素，函数返回 [constEnd](QMap.md#qmapconstiterator-qmapconstend-const)().

Qt 4.1 中引入该函数。

**另请参阅** [find](QMap.md#qmapiterator-qmapfindconst-key-key)() 和 [QMultiMap::constFind](../../M/QMultiMap/QMultiMap.md#typename-qmapkey-tconstiterator-qmultimapconstfindconst-key-key-const-t-value-const)()。

### [QMap::const_key_value_iterator](QMap.md#typedef-qmapconstkeyvalueiterator) QMap::constKeyValueBegin() const

返回常量类型的 [STL 风格迭代器](../../C/Container_Classes/Container_Classes.md#STL-风格迭代器) 指向 map 中的第一项.

Qt 5.10 中引入该函数。

**另请参阅** [keyValueBegin](QMap.md#qmapkeyvalueiterator-qmapkeyvaluebegin)()。

### [QMap::const_key_value_iterator](QMap.md#typedef-qmapconstkeyvalueiterator) QMap::constKeyValueEnd() const

返回常量类型的 [STL 风格迭代器](../../C/Container_Classes/Container_Classes.md#STL-风格迭代器) 指向 map 中最后一项之后的假想项。

Qt 5.10 中引入该函数。

**另请参阅** [constKeyValueBegin](QMap.md#qmapconstkeyvalueiterator-qmapconstkeyvaluebegin-const)()。

### bool QMap::contains(const Key &*key*) const

如果该 map 含有键为 key 的元素，返回 `true`；否则返回 `false`。
...
提供该函数以兼容 STL。等同于 [isEmpty](QMap.md#bool-qmapisempty-const)()，如果 map 为空，返回 `true`；否则返回 `false`。
...
返回指向 map 中键为 key 的元素的迭代器。

如果 map 中没有键为 key 的元素，函数返回 [end](QMap.md#qmapiterator-qmapend)()。
...
**另请参阅** [constFind](QMap.md#qmapconstiterator-qmapconstfindconst-key-key-const)(), [value](QMap.md#const-t-qmapvalueconst-key-key-const-t-defaultvalue--t-const)(), [values](QMap.md#qlistt-qmapvalues-const)(), [lowerBound](QMap.md#qmapiterator-qmaplowerboundconst-key-key)(), [upperBound](QMap.md#qmapiterator-qmapupperboundconst-key-key)() 和 [QMultiMap::find](../../M/QMultiMap/QMultiMap.md#typename-qmapkey-titerator-qmultimapfindconst-key-key-const-t-value)()。
...
返回指向 map 中第一个值的引用，即映射到键最小的值。该函数假定 map 不为空。

对于非共享map，该函数在[常数时间](../../C/Container_Classes/Container_Classes.md#算法复杂度)完成。
...
该操作花费[常数时间](../../C/Container_Classes/Container_Classes.md#算法复杂度)。

Qt 5.2 中引入该函数。

**另请参阅** [lastKey](QMap.md#const-key-qmaplastkey-const)(), [first](QMap.md#t-qmapfirst)(), [keyBegin](QMap.md#qmapkeyiterator-qmapkeybegin-const)() 和 [isEmpty](QMap.md#bool-qmapisempty-const)()。

### [QMap::iterator](../../M/QMap/qmap-iterator.md) QMap::insert(const Key &*key*, const T &*value*)

以键 key 和值 value 插入一个新元素。
...
以键 key 和值 value 插入一个新元素，pos 用来提示插入位置。

如果以 [constBegin](QMap.md#qmapconstiterator-qmapconstbegin-const)() 作为插入位置提示，表明 key 比 map 中的任何键都小，而 [constEnd](QMap.md#qmapconstiterator-qmapconstend-const)() 则建议 key （严格）大于 map 中的任何键。其它情况提示应该满足条件 (pos - 1).[key](QMap.md#const-key-qmapkeyconst-t-value-const-key-defaultkey--key-const)() < key <= pos.[key](QMap.md#const-key-qmapkeyconst-t-value-const-key-defaultkey--key-const)()。如果提示 pos 是错误的，它将被忽略，以常规方式插入。

如果已经存在键为 key 的元素，该元素的值将被 value 替换。

如果有多个键为 key 的元素，只会有一个元素的值被 value 替换。

如果提示是正确的，并且 map 未被共享，插入平均以[常数时间复杂度](../../C/Container_Classes/Container_Classes.md#算法复杂度)执行。

从有序数据创建 map 时，从最大键的元素开始以 [constBegin](QMap.md#qmapconstiterator-qmapconstbegin-const)() 作为提示插入，比按从小到大的顺序以 [constEnd](QMap.md#qmapconstiterator-qmapconstend-const)() 作为提示插入更快，因为 [constEnd](QMap.md#qmapconstiterator-qmapconstend-const)() - 1 （用来检查提示是否合法）需要[对数时间](../../C/Container_Classes/Container_Classes.md#算法复杂度)。

注意：需小心对待提示。提供从旧的共享实例取得的迭代器可能引起崩溃，还会有污染 map 和 pos 映射的风险。
...
插入传入的 *map* 中的所有元素到该 map 中。

如果一个键同时在两个 map 中出现，其值将被传入的 *map* 中的值替换。

**注意:** 如果传入的 *map* 同一键包含多个元素，则该键的最终值未定义。
...
如果 map 中不包含任何项目，返回 `true`；否则返回 `false`。
...
返回与值 *value* 对应的第一个键，如果 map 中不包含值为 *value* 的元素，返回 *defaultKey*。如果没有提供 *defaultKey*，函数返回 [默认构造的键](../../C/Container_Classes/Container_Classes.md#容器类)。

该函数可能会很慢 ([线性时间](../../C/Container_Classes/Container_Classes.md#算法复杂度))，因为 [QMap](../../M/QMap/QMap.md) 的内部数据结构是以快速查找键而不是值为目标来优化的。
...
**另请参阅** [keyEnd](QMap.md#qmapkeyiterator-qmapkeyend-const)() 和 [firstKey](QMap.md#const-key-qmapfirstkey-const)()。
...
**另请参阅** [keyBegin](QMap.md#qmapkeyiterator-qmapkeybegin-const)() 和 [lastKey](QMap.md#const-key-qmaplastkey-const)()。

### [QMap::key_value_iterator](QMap.md#typedef-qmapkeyvalueiterator) QMap::keyValueBegin()

返回 [STL 风格迭代器](../../C/Container_Classes/Container_Classes.md#STL-风格迭代器) 指向 map 中的第一项。

Qt 5.10 中引入该函数。

**另请参阅** [keyValueEnd](QMap.md#qmapkeyvalueiterator-qmapkeyvalueend)().

### [QMap::const_key_value_iterator](QMap.md#typedef-qmapconstkeyvalueiterator) QMap::keyValueBegin() const
...
### [QMap::key_value_iterator](QMap.md#typedef-qmapkeyvalueiterator) QMap::keyValueEnd()
...
### [QMap::const_key_value_iterator](QMap.md#typedef-qmapconstkeyvalueiterator) QMap::keyValueEnd() const
...
键的顺序保证和通过 [values](QMap.md#qlistt-qmapvalues-const)() 返回的值的顺序相同。
...
返回 map 中最后一个值的引用，即最大键所对应的值。该函数假定 map 不为空。

对于非共享 map（或常量类型的 map)，该函数以 [对数时间](../../C/Container_Classes/Container_Classes.md#算法复杂度) 执行。
...
该函数以 [对数时间](../../C/Container_Classes/Container_Classes.md#算法复杂度) 执行。

Qt 5.2 中引入该函数。

**另请参阅** [firstKey](QMap.md#const-key-qmapfirstkey-const)(), [last](QMap.md#t-qmaplast)(), [keyEnd](QMap.md#qmapkeyiterator-qmapkeyend-const)() 和 [isEmpty](QMap.md#bool-qmapisempty-const)()。

### [QMap::iterator](../../M/QMap/qmap-iterator.md) QMap::lowerBound(const Key &*key*)

返回指向 map 中键 key 所关联的元素中第一个元素的迭代器。如果 map 中不包含键为 key 的元素，函数返回指向距离下一个键最近的元素的迭代器。
...
如果 map 中存在多个键为 key 元素，该函数返回指向最新插入的值的迭代器。其它值可以通过递增迭代器访问。例如，下面的例子遍历同一键所关联的所有元素：
...
从 map 中移除所有键为 *key* 的元素。返回被移除的元素个数，如果键存在个数为1，否则为0。
...
将 other map 与本 map 交换。该操作非常快，永远不失败。
...
如果 map 中不存在该元素，函数简单返回一个[默认构造出来的值](../../C/Container_Classes/Container_Classes.md#容器类)。如果同一键有多个元素，只移除最新插入的元素并返回值。
...
返回指向 map 中首个大于键 key 的元素的迭代器。如果 map 中不存在键为 key 的元素，该函数返回指向距离下一个键最近的元素的迭代器。
...
如果 map 中不包含键为 key 的元素，该函数返回 defaultValue。 如果没有指定 defaultValue，该函数返回[默认构造的值](../../C/Container_Classes/Container_Classes.md#容器类)。如果同一键在 map 中有多个元素，返回最新插入的元素的值。
...
按照键升序返回包含 map 中所有值的列表。如果键关联到多个值，所有值都会放到列表中，不只是最新插入的那个。
...
如果 other 与 map 不相等，返回 `true`，否则返回 `false`。
...
如果 other 与 map 相等，返回 `true`，否则返回 `false`。
